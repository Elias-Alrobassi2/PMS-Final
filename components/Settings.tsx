...
const Settings: React.FC = () => {
  const { settings, setSettings, backupData, restoreData, currentUser } = useContext(AppContext);
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <select 
                    ...
                    className="mt-1 block w-full p-2 border rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600"
                >
          ...
          <div>
            ...
            <div className="mt-2 flex gap-4">
              {(['light', 'dark', 'system'] as Theme[]).map(theme => (
                <button key={theme} onClick={() => handleSettingChange('theme', theme)} className={`px-4 py-2 rounded-md font-semibold text-sm ${settings.theme === theme ? 'bg-accent-blue text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
          ...
          <div>
            ...
            <div className="mt-2 flex gap-3">
              {THEME_COLORS.map(color => (
                <button key={color.name} onClick={() => handleSettingChange('accentColor', color.name as AccentColor)} className={`w-10 h-10 rounded-full border-4 ${settings.accentColor === color.name ? 'border-accent-blue' : 'border-transparent'}`} style={{ backgroundColor: color.hex }} title={color.label} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <button onClick={backupData} className="flex-1 bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">
            نسخ احتياطي للبيانات
          </button>
          <label className="flex-1 bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 cursor-pointer text-center">
            استعادة البيانات
